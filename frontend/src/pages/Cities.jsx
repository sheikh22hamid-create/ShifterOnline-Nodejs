...
import CityFormModal from '../components/master/CityFormModal'
...
export default function Cities() {
  ...
  const fetcher = useCallback(() => api.get('/cities').then((res) => res.data), [])
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setDeleteError(err.response?.data?.message || 'Could not delete this city.')
    } finally {
      setDeleting(false)
    }
  }
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-[13px]" style={{ color: 'var(--ink-muted)' }}>
            Drives every city admin's and executive's scoping.
          </p>
        </div>
        {canManage && (
          <button
          ...
          <table className="w-full text-left text-[13px]">
            <thead>
              <tr style={{ background: 'var(--bg)' }}>
                {['City', 'Coordinates', 'Status', canManage ? '' : undefined].filter((h) => h !== undefined).map((h) => (
      ...
      {canManage && (
        <>
          <CityFormModal
            ...
            onSaved={() => {
              toast.success(formTarget ? 'City updated.' : 'City created.')
          ...
          >
            ...
            <p className="text-[13px]" style={{ color: 'var(--ink-muted)' }}>
              Delete <strong style={{ color: 'var(--ink)' }}>{deleteTarget?.title}</strong>? Blocked if it still has staff, drivers, orders, or customers — deactivate instead in that case.
            </p>
          </Modal>
        </>
      )}

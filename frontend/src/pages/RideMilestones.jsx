...
import { Plus, Pencil, Trash2, Milestone } from 'lucide-react'
...
function MilestoneFormModal({ open, milestone, onClose, onSaved }) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={milestone ? 'Edit ride milestone' : 'New ride milestone'}
      footer={
        <>
...
export default function RideMilestones() {
  ...
  const fetcher = useCallback(() => api.get('/ride-milestones').then((res) => res.data), [])
  ...
  async function handleDelete() {
    setDeleting(true)
    try {
      await api.delete(`/ride-milestones/${deleteTarget.id}`)
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-[13px]" style={{ color: 'var(--ink-muted)' }}>
            Automatically reward every customer with a plan once they cross a set number of lifetime completed rides.
          </p>
        </div>
        {canManage && (
          <button
                  ...
                  {canManage && (
                    <td className="px-4 py-2.5">
                      <div className="flex justify-end gap-2">
                        <button type="button" onClick={() => setFormTarget(m)} style={{ color: 'var(--ink-faint)' }} aria-label="Edit">
                          <Pencil size={14} />
                        </button>
                        <button type="button" onClick={() => setDeleteTarget(m)} style={{ color: 'var(--danger)' }} aria-label="Delete">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </td>
      ...
      {canManage && (
        <>
          <MilestoneFormModal
            ...
            milestone={formTarget}
            onClose={() => setFormTarget(undefined)}
            onSaved={() => {
              toast.success(formTarget ? 'Milestone updated.' : 'Milestone created.')
          ...
          >
            <p className="text-[13px]" style={{ color: 'var(--ink-muted)' }}>
              Delete the <strong style={{ color: 'var(--ink)' }}>{deleteTarget?.rides_required} rides</strong> milestone? This can't be undone.
            </p>
          </Modal>
        </>
      )}

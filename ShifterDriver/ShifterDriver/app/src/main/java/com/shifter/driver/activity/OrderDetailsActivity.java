...
public class OrderDetailsActivity extends AppCompatActivity
    ...
    // screen to show (advance payment was never collected yet), so onCreate
    // renders it immediately instead of blocking the first frame on the
    // pkgHistory round-trip below — that network call was the entire ~10s
    // delay between tapping Accept and the waiting screen appearing
    // (confirmed live). The pkgHistory check still runs, just in the
    // background via the same 2s polling this screen already does, so a
    // same-instant edge case (already paid / already cancelled) still
    // self-corrects within moments instead of gating the first paint.
    private static final String EXTRA_JUST_ACCEPTED = "just_accepted";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d("TimingProbe", "ORDER_DETAILS_ON_CREATE t=" + System.currentTimeMillis());
    ...
    private void showWaitingForPaymentScreen(com.google.gson.JsonObject rootObj, PDOrder pdOrder) {
        Log.d("TimingProbe", "WAITING_SCREEN_SET_CONTENT t=" + System.currentTimeMillis());
    ...
    private void fetchCompletedOrderAndShowDialog(String orderId) {
        // Up to 2 retries before giving up. The backend write (order_status
        // 'Completed' + the commission/wallet debit) is fully committed
        // before this call ever fires — orderSocket's ack only goes out
        // after tripLifecycle.updateStatus resolves — so a miss here is a
        // transient network/read hiccup, not a real absence. Falling back
        // straight to `orderItem` (built for live tracking, never carries
        // commission/cash_to_collect/driver_total_earning) is what made the
        // popup show the full raw fare as "Cash to Collect" with the whole
        // Deductions section blank on a single failed attempt (confirmed
        // live on order #1765: server-side data was correct the whole time).
        fetchCompletedOrderAndShowDialog(orderId, 2);
    }

    private void fetchCompletedOrderAndShowDialog(String orderId, int retriesLeft) {
        custPrograssbar.prograssCreate(this);
        JSONObject jsonObject = new JSONObject();
        try {
        ...
        call.enqueue(new retrofit2.Callback<JsonObject>() {
            ...
            public void onResponse(Call<JsonObject> call, retrofit2.Response<JsonObject> response) {
                ...
                if (completedItem == null && retriesLeft > 0) {
                    new android.os.Handler(getMainLooper()).postDelayed(
                            () -> fetchCompletedOrderAndShowDialog(orderId, retriesLeft - 1), 1200);
                    return;
                }
    ...
    private void showCompletedOrderDialog(PDOrderItem orderItem) {
        ...
        // Premium plan benefit actually applied to this ride (driver's own
        // active plan, e.g. 0% commission override) — only shown when one
        // was genuinely selected server-side, never a static plan-card promise.
        View planBenefitStrip = dialog.findViewById(R.id.layout_plan_benefit_strip);
        android.widget.TextView txtPlanBenefit = dialog.findViewById(R.id.txt_plan_benefit);
        if (planBenefitStrip != null && txtPlanBenefit != null) {
            if (orderItem.isPlanBenefitApplied() && orderItem.getPlanName() != null && !orderItem.getPlanName().trim().isEmpty()) {
                ...
                if (planDiscount > 0) {
                    msg.append(" — ").append(currency)
                            .append(String.format(java.util.Locale.getDefault(), "%.2f", planDiscount))
                            .append(" saved on commission");
                }
                if (planIncentive > 0) {
                    msg.append(planDiscount > 0 ? ", plus " : " — ").append(currency)
                            .append(String.format(java.util.Locale.getDefault(), "%.2f", planIncentive))
                            .append(" incentive earned");
                }
                txtPlanBenefit.setText(msg.toString());
                planBenefitStrip.setVisibility(View.VISIBLE);
            } else {
                planBenefitStrip.setVisibility(View.GONE);
            }
        }

...
async function registerDevice({ uid, deviceId, fcmToken, platform, deviceName, appVersion }) {
  ...
  try {
    const oldDevices = await prisma.tbl_user_device.findMany({
      where: { uid, device_id: { not: deviceId }, is_active: true },
      select: { fcm_token: true },
    });
    const oldTokens = oldDevices.map((d) => d.fcm_token).filter(Boolean);

    await prisma.tbl_user_device.updateMany({
      where: { uid, device_id: { not: deviceId }, is_active: true },
      data: { is_active: false, logged_out_at: new Date() },
    });

    const existing = await prisma.tbl_user_device.findFirst({ where: { uid, device_id: deviceId } });
    if (existing) {
      await prisma.tbl_user_device.update({
      ...
      await prisma.tbl_user_device.create({
        data: {
          uid,
          device_id: deviceId,

...
function buyOrderItemUpload(req, res) {
  buyOrderItemUploadMulter(req, res, async (err) => {
    ...
    try {
      ...
      // IDOR guard: same "trust but verify when given" pattern used
      // elsewhere in this codebase - rider_id IS sent here, so cheaply
      // confirm this order is actually assigned to the calling rider before
      // letting them overwrite another rider's item image/total.
      const order = await prisma.buy_order.findUnique({ where: { id: orderId }, select: { rid: true } });
      if (!order || order.rid !== riderId) {
        return res.status(200).json({ ResponseCode: "401", Result: "false", ResponseMsg: "Something Went Wrong!" });
      }
  ...
  try {
    ...
    return res.status(200).json({
      ...
      ResponseMsg: isEligible
        ...
        : "Next Day Delivery is exclusively available for Premium Plan members.",

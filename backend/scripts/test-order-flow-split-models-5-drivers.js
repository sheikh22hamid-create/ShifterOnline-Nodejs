 ...
 *     - Drivers #01, #02: ONLY Model 1 + Model 2 ON (Models 3,4,5 OFF)
...
// Driver #01, #02 -> Model 1 + 2 ON. Driver #03, #04, #05 -> Model 3 + 4 + 5 ON.
...
async function prepare5Drivers() {
  ...
  const otherBikeRiders = await prisma.tbl_rider.findMany({
    where: {
      vehicle: "Bike",
      a_status: 1,
      NOT: { vehicle_no: { startsWith: "TEST-SPLIT-" } },
    },
  ...
  testRiders = [];
  for (let i = 1; i <= 5; i++) {
    const vehicleNo = `TEST-SPLIT-${pad2(i)}`;
...
function printSummaryTable(eventsMap) {
  console.log("\n=========================================================================================================");
  console.log("📊 SPLIT MODELS (2 drivers: Model1+2 | 3 drivers: Model3+4+5) DISPATCH TIMELINE REPORT");
  console.log("=========================================================================================================");
...
async function main() {
  ...
  try {
    ...
    const EXPECTED_TOTAL_EVENTS = 13;
